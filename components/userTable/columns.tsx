...
import { onDeleteUser } from "@/Functions/Function";
...
const DeleteUser = async(email: string) =>{
  if(confirm("Do you want to delete ?")){
    console.log("Delete Function: "+email);
    const status = await onDeleteUser(email);
    if(status.success){
      alert("User Deleted, Refresh your page !!")
    }else{
...
export const columns: ColumnDef<StudentData>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className="uppercase font-semibold">#{row.getValue("id")}</div>
    ),
  },
  {
    accessorKey: "name",
  ...
  },
  {
    ...
    header: "Actions",
    cell: ({ row }) => (
      <Button onClick={() => DeleteUser(row.getValue("email"))}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="currentColor"
